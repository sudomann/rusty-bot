...
pub enum Accuracy {
    ...
    Rough,
    /// Same as [`Accuracy::Rough`], but with shortened output text
    RoughShort,
...
impl TimePeriod {
    ...
    fn to_text_rough_short(self) -> Cow<'static, str> {
        match self {
            Self::Now => "now".into(),
            Self::Nanos(n) => format!("{}ns", n).into(),
            Self::Micros(n) => format!("{}µs", n).into(),
            Self::Millis(n) => format!("{}ms", n).into(),
            Self::Seconds(n) => format!("{}s", n).into(),
            Self::Minutes(n) => format!("{}m", n).into(),
            Self::Hours(n) => format!("{}h", n).into(),
            Self::Days(n) => format!("{}d", n).into(),
            Self::Weeks(n) => format!("{}w", n).into(),
            Self::Months(n) => format!("{}m", n).into(),
            Self::Years(n) => format!("{}y", n).into(),
            Self::Eternity => "∞".into(),
        }
    }
    ...
    fn to_text(self, accuracy: Accuracy) -> Cow<'static, str> {
        match accuracy {
            Accuracy::RoughShort => self.to_text_rough_short(),
...
impl HumanTime {
    ...
    pub fn to_text_en(self, accuracy: Accuracy, tense: Tense) -> String {
        let mut periods = match accuracy {
            Accuracy::RoughShort => self.rough_period(),
